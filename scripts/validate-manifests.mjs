...
function resolveRefs(obj, baseDir) {
  ...
  if (Array.isArray(obj)) {
    obj.forEach(item => resolveRefs(item, baseDir));
    return;
  }
  ...
  Object.values(obj).forEach(value => resolveRefs(value, baseDir));
}

/**
 * Validate a single manifest file (individual JSON file)
 */
function validateSingleFile(filePath, schema) {
...
function validateManifestDirectory(dirName, schemaFile) {
  ...
  for (const file of files) {
    ...
    const result = validateSingleFile(filePath, schema);
