...
export default async function VendorPage({ params }: { params: Promise<{ locale: string; slug: string }> }) {
  const { locale, slug } = await params;
  ...
  return (
    <>
      ...
      {/* Community Links */}
      {vendor.communityUrls && Object.entries(vendor.communityUrls).filter(([, url]) => url).length > 0 && (
        <LinkCardGrid
