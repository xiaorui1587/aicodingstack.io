...
export async function generateMetadata({ params }: { params: Promise<{ locale: string }> }) {
  ...
  return await generateListPageMetadata({
    ...
    additionalKeywords: ['LLM vendors', 'Google', 'Microsoft', 'AI companies', 'foundation model providers'],
  });
}
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-[var(--spacing-md)]">
              {localizedVendors.map((vendor) => (
                ...
                >
                  ...
                  <p className="text-sm leading-relaxed text-[var(--color-text-secondary)] font-light min-h-[4rem]">{vendor.description}</p>
                </Link>
              ))}
            </div>
          </main>
