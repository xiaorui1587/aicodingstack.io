 ...
 */
export function buildOGImage(options: {
  imagePath: string;
  alt: string;
}): {
  url: string;
 ...
 */
export function formatPriceForDescription(pricing: Array<{ value: number | null; currency: string | null; per: string | null }> | undefined): string | null {
  ...
  const price = firstTier.currency ? `${firstTier.currency} ${firstTier.value}` : `${firstTier.value}`;
